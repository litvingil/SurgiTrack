...
if __name__ == '__main__':
    ...
    if args.input:
        ...
        for path in tqdm.tqdm(args.input):
            ...
            feature_files.append(os.path.join(args.output, os.path.basename(path).split('.')[0] + '.npy'))
            
    feats = []
    for feat in sorted(feature_files):
        feats.append(torch.torch.from_numpy(np.load(feat)))
    feats = torch.cat(feats, dim=0)

    # compute cosine distance
    norm = torch.norm(feats, p=2, dim=1, keepdim=True)
    normalized_x = feats / norm
    distmat = 1 - torch.mm(normalized_x, normalized_x.t())
    distmat = distmat.numpy()
    plt.imshow(distmat)

    plt.savefig('/home/user_219/omer_gil/one_camera_tracking/Visualizations/distances_reid_fullbody.png', dpi=1000)
